...
response = requests.get(f"{server}/{email}/register?email={tex2}&encrypted=true")
...
response1 = requests.get(f"{server}/{email}/register?email={tex1}&encrypted=true")
print(response1.text)
decoded = base64.b64decode(response1.text)
print("len decoded", len(decoded))
for i in range(5):
  print(response1.text[i*16:(i+1)*16])
print(challenge2[16:32] + challenge2[:16]+ challenge2[-32:-16] + response1.text[-16:])
